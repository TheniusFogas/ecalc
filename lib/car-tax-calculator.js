...
export class CarTaxCalculator {
  ...
  // ⚠️ FLAG (audit 2026-08-11): these figures were reported by an independent review as
  // roughly 2× the historical Art. 470 base rates (e.g. ~8 lei/200cm³ for the lowest sub-1600cc
  // tier in older published tables vs 19.5 here) — but rates ARE periodically re-indexed, and
  // this reviewer does not have high enough confidence in the exact current Monitorul Oficial
  // figures to safely overwrite them (unlike the OUG 156/2024 salary-facility changes fixed
  // elsewhere in this pass, which is a well-documented, high-confidence fact). Left UNCHANGED
  // on purpose rather than risk swapping one wrong number for another guessed one — please
  // verify this table against the current Codul Fiscal / ANAF before relying on it.
