...
export class CarTaxCalculator {
  ...
  // ✅ VERIFIED (audit 2026-08-12): initially flagged as suspiciously ~2× older remembered
  // figures. Checked against a live current source (noulcodfiscal.ro, Art. 470 alin. (2), the
  // consolidated Codul Fiscal text) — these rates match exactly. The base rates from Art. 470
  // are re-indexed annually for inflation, which is why they look roughly double what older
  // (pre-indexation) tables show. Table below is correct as of the current law; if this drifts
  // out of date in a future year, re-check against the same source.
