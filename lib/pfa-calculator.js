...
export class PFACalculator {
  ...
  calculateCASThreshold(yearlyIncome) {
    ...
    // FIX (audit 2026-08-11): was `minSalary * 12 * (cas_obligatory_12 || 12)` — an extra
    // stray "* 12" turned the real threshold ("12 salarii minime pe an", ≈48.600 RON at a
    // 4050 RON minimum wage) into ≈583.200 RON, meaning almost no PFA ever hit the mandatory
    // CAS threshold. Matches the pattern already used correctly for CASS just above
    // (minSalary * cass_min_threshold, no extra factor).
    const threshold12 = minSalary * (this.rules.cas_obligatory_12 || 12);
    const threshold24 = minSalary * (this.rules.cas_obligatory_24 || 24);
  ...
  calculateSRL(yearlyRevenue, yearlyExpenses = 0, employeesCount = 0, options = {}) {
    ...
    // FIX (audit 2026-08-11): default fallback updated 8 → 10 — impozitul pe dividende a
    // crescut la 10% începând cu ianuarie 2025. Câmpul e acum și configurabil din admin
    // (PFA → Praguri și Limite → Impozit Dividende), în loc să fie doar hardcodat aici.
    const dividendTaxRate = this.rules.dividend_tax_rate || 10;
